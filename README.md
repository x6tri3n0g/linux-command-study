...
`cat`은 파일의 전체 내용을 출력해줍니다. `head`는 파일의 앞부분 일부를 출력해주고, `tail`은 뒤에서 일부분을 출력해줍니다.
...
## 9. `-h`, `-help` : 헬프 옵션으로 사용법 출력하기

명령어들의 사용법이 궁금하거나, 어떤 기능이나 옵션이 있는지 궁금하다면 도움말을 출력해볼 수 있습니다.
도움말을 출력하는 방법은 크게 3가지가 있습니다.

1. 아무런 인자없이 명령어를 실행한다.
2. `-h` 옵션을 붙여서 실행한다.
3. `--help` 옵션을 붙여서 실행한다(가끔은 `-help`인 경우도 있습니다).
